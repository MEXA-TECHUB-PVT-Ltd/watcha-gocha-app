...
export default function SearchScreenPicTours({navigation}) {
  ...
  const fetchItemData = async (search) => {
    ...
    try {
      const response = await fetch(
        `https://watch-gotcha-be.mtechub.com/picTour/searchTour?name=${search}`,
        {
          method: 'GET',
          headers: {
      ...
      console.log('AllItems', result.Tours);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(
          `https://watch-gotcha-be.mtechub.com/picTour/searchTour?name=${selectedItemId}`,
          {
            method: 'GET',
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const loadSearchesFromStorage = async () => {
    try {
      const savedSearches = await AsyncStorage.getItem('searchPics');
      if (savedSearches) {

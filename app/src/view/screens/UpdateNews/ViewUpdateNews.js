...
export default function ViewUpdateNews({navigation, route}) {
...
//------------------------------------\\
  ...
  console.log('Recieved Data News', receivedData?.news_id);
  ...
  const changeModals = () => {
    ...
    navigation.navigate('UpdatePostOnNews', { item:receivedData});
  };
  ...
  const handleUpdateDelete = async () => {
    ...
    try {
      const response = await fetch(`https://watch-gotcha-be.mtechub.com/news/deleteNews/${receivedData?.news_id}`, {
        method: 'DELETE',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error(`Error deleting video with ID ${receivedData?.news_id}:`, response.status);

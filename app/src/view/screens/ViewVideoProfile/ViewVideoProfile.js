...
export default function ViewVideo({navigation, route}) {
  ...
  const fetchLikes = async result => {
    ...
    try {
      const response = await fetch(
        `https://watch-gotcha-be.mtechub.com/xpi/getAllLikesByVideo/${receivedData.video_id}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleUpdateDelete = async () => {
    ...
    try {
      const response = await fetch(`https://watch-gotcha-be.mtechub.com/xpi/deleteXpiVideo/${receivedData?.video_id}`, {
        method: 'DELETE',
        headers: {
